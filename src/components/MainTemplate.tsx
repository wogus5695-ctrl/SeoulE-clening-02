...
export default function MainTemplate({ 
  ...
  // 청소 범위 데이터 (3개 서클)
  const getCleaningScopeCircles = () => {
    if (customContent?.cleaningScope?.circles && customContent.cleaningScope.circles.length >= 3) {
      return customContent.cleaningScope.circles;
    }
    const defaultScope = [
      { title: '현장 상태', desc: '면적 · 구조 · 접근성' },
      { title: '오염 상태', desc: '먼지 · 기름때 · 생활오염' },
      {
        title: '작업 종류',
        desc: activeServiceObj?.commonProblems && activeServiceObj.commonProblems.length > 0 
          ? activeServiceObj.commonProblems.join(' · ') 
          : '외벽 · 유리창 · 바닥 · 준공\n후드 · 특수청소'
      }
    ];
    return defaultScope;
  ...
  const getIntersectionText = () => {
    return customContent?.cleaningScope?.intersectionText || '맞춤 작업\n범위 안내';
  };

  const getPricingFooterDesc = () => {
    if (customContent?.pricingFactors?.description) {
      return customContent.pricingFactors.description;
    }
    return '💡 전화 상담 시 지역과 필요한 청소 종류만 알려주셔도 작업 가능 여부부터 확인해드립니다.';
  };

  const getProcessSteps = () => {
    if (customContent?.processSteps && customContent.processSteps.length > 0) {
      return customContent.processSteps;
    }
    if (activeServiceObj?.process && activeServiceObj.process.length > 0) {
      return activeServiceObj.process.map((stepStr, idx) => ({
        step: `0${idx + 1}`.slice(-2),
        title: stepStr,
        desc: activeServiceObj.serviceNameKo + ' 진행 단계'
      }));
    }
    return [
  ...
  const getFaqList = () => {
    if (customContent?.faq && customContent.faq.length > 0) {
      return customContent.faq.map(f => ({ q: f.q, a: f.a }));
    }
    if (activeServiceObj?.faq && activeServiceObj.faq.length > 0) {
      return activeServiceObj.faq.map(f => ({ q: f.question, a: f.answer }));
    }
    return [
      {
          ...
          <div className={styles.heroTip}>
            <span>
              {keywordObj || regionObj || region !== '서울·경기' || service !== '종합청소'
                ? '💡 현장 사진 and 위치를 알려주시면 작업 가능 여부와 상담 방향을 빠르게 안내해 드립니다.'
  ...
  const renderServices = (sectionTitleText: string) => (
      ...
      <div className={styles.inner}>
          ...
          <div className={styles.diagramContainer}>
            ...
            <div className={`${styles.circle} ${styles.circle1}`}>
              <div className={styles.circleText}>
                <h3>{getCleaningScopeCircles()[0].title}</h3>
                <p>{getCleaningScopeCircles()[0].desc}</p>
              </div>
            </div>
            ...
            <div className={`${styles.circle} ${styles.circle2}`}>
              <div className={styles.circleText}>
                <h3>{getCleaningScopeCircles()[1].title}</h3>
                <p>{getCleaningScopeCircles()[1].desc}</p>
              </div>
            </div>
            ...
            <div className={`${styles.circle} ${styles.circle3}`}>
              <div className={styles.circleText}>
                <h3>{getCleaningScopeCircles()[2].title}</h3>
                <p style={{ whiteSpace: 'pre-line' }}>{getCleaningScopeCircles()[2].desc}</p>
              </div>
            </div>
            
            {/* Center intersection circle */}
            <div className={styles.intersection}>
              {getIntersectionText().split('\n').map((line, lIdx) => (
                <span key={lIdx}>
                  {line}
                  {lIdx === 0 && <br />}
                </span>
        ...
        <div className={styles.mobileStepsWrapper}>
          <div className={styles.mobileSteps}>
            {getCleaningScopeCircles().map((circle, idx) => (
            ...
            <div className={styles.mobileResultCard}>
              <h3>{getIntersectionText().replace(/\n/g, ' ')}</h3>
            </div>
          </div>
          {!isNewService && (
            <p className={styles.diagramFooterDesc}>
      ...
      <div className={styles.inner} style={{ position: 'relative', zIndex: 2 }}>
        <div className={styles.processWrapper}>
          <div className={styles.sectionHeader}>
            ...
            <p className={styles.sectionDesc}>과도한 추가금 요구 없는 투명하고 체계적인 진행 방식입니다.</p>
          </div>
          <div className={styles.timelineContainer}>
            <div className={styles.timelineLine}></div>
            <div className={styles.processSteps}>
              {getProcessSteps().map((item, idx) => (
  ...
  const renderPricingFactors = () => (
    <section style={{ padding: '60px 0', backgroundColor: 'var(--gray-50)', borderTop: '1px solid var(--gray-200)', borderBottom: '1px solid var(--gray-200)' }}>
      <div className={styles.inner}>
        <div className={styles.sectionHeader}>
          <span className={styles.subTitle}>PRICING FACTORS</span>
          <h2 className={styles.sectionTitle}>견적 결정 요소</h2>
          <p className={styles.sectionDesc}>현장 상황에 맞는 합리적인 가격 기준을 투명하게 안내합니다.</p>
        </div>
        <div style={{ maxWidth: '800px', margin: '0 auto', backgroundColor: 'white', padding: '24px 30px', borderRadius: '12px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: '1px solid var(--gray-200)', whiteSpace: 'pre-line', fontSize: '15px', lineHeight: '1.7', color: 'var(--gray-700)' }}>
          {getPricingFooterDesc()}
        </div>
      </div>
    </section>
  ...
  const renderRelatedLinks = () => (
    customContent?.relatedLinks && customContent.relatedLinks.length > 0 && (
      <section style={{ padding: '40px 0', backgroundColor: 'var(--gray-50)', borderTop: '1px solid var(--gray-200)', borderBottom: '1px solid var(--gray-200)' }}>
        <div className={styles.inner} style={{ textAlign: 'center' }}>
          <h3 style={{ fontSize: '18px', fontWeight: '800', marginBottom: '20px', color: 'var(--primary)' }}>추천 관련 서비스</h3>
          <div style={{ display: 'flex', justifyContent: 'center', gap: '15px', flexWrap: 'wrap' }}>
            {customContent.relatedLinks.map((link, lIdx) => (
              <Link 
                key={lIdx} 
                href={link.path}
                style={{
